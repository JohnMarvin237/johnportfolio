'use client';
// Client Component — needed for useTranslation (React Context hook) and framer-motion hooks
import { motion, useReducedMotion } from 'framer-motion';
import Image from 'next/image';
import Button from '../ui/Button';
import Link from 'next/link';
import { useTranslation } from '@/lib/i18n/LanguageContext';

interface HeroProps {
  name?: string;
  imageUrl?: string;
  githubUrl?: string;
  linkedinUrl?: string;
}

export default function Hero({
  name = 'John Doe',
  imageUrl,
  githubUrl = 'https://github.com',
  linkedinUrl = 'https://linkedin.com',
}: HeroProps) {
  const { t } = useTranslation();
  const shouldReduceMotion = useReducedMotion();

  // When prefers-reduced-motion is active, use the same values for initial
  // and animate so no visual motion occurs.
  function fadeUp(delaySeconds: number) {
    return {
      initial: shouldReduceMotion ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 },
      animate: { opacity: 1, y: 0 },
      transition: { duration: 0.5, ease: 'easeOut' as const, delay: shouldReduceMotion ? 0 : delaySeconds },
    };
  }

  function fadeOnly(delaySeconds: number) {
    return {
      initial: shouldReduceMotion ? { opacity: 1 } : { opacity: 0 },
      animate: { opacity: 1 },
      transition: { duration: 0.5, ease: 'easeOut' as const, delay: shouldReduceMotion ? 0 : delaySeconds },
    };
  }

  return (
    <section className="relative bg-gradient-to-b from-primary-50 to-white dark:from-gray-900 dark:to-gray-900 py-20 sm:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="lg:grid lg:grid-cols-2 lg:gap-8 items-center">
          <div className="text-center lg:text-left">
            <motion.div {...fadeUp(0)}>
              <h1 className="text-4xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-6xl">
                {t('hero.greeting')}{' '}
                <span className="text-primary-600">{name}</span>
              </h1>
            </motion.div>

            <motion.div {...fadeUp(0.1)}>
              <p className="mt-4 text-2xl text-gray-600 dark:text-gray-400">
                {t('hero.title')}
              </p>
            </motion.div>

            <motion.div {...fadeUp(0.2)}>
              <p className="mt-6 text-lg leading-8 text-gray-600 dark:text-gray-400">
                {t('hero.description')}
              </p>
            </motion.div>

            <motion.div {...fadeUp(0.3)}>
              <div className="mt-10 flex items-center justify-center gap-x-6 lg:justify-start">
                <Link href="/projects">
                  <Button size="lg">{t('hero.viewProjects')}</Button>
                </Link>
                <Link href="/contact">
                  <Button variant="outline" size="lg">{t('hero.contactMe')}</Button>
                </Link>
              </div>
            </motion.div>

            <motion.div {...fadeUp(0.4)}>
              <div className="mt-10 flex items-center justify-center gap-x-6 lg:justify-start">
                <a
                  href={githubUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors"
                  aria-label="GitHub"
                >
                  <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" />
                  </svg>
                </a>
                <a
                  href={linkedinUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors"
                  aria-label="LinkedIn"
                >
                  <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                  </svg>
                </a>
              </div>
            </motion.div>
          </div>

          {imageUrl && (
            <motion.div {...fadeOnly(0.3)} className="mt-10 lg:mt-0">
              <div className="relative mx-auto w-64 h-64 sm:w-80 sm:h-80 lg:w-full lg:h-full max-w-md">
                <div className="absolute inset-0 bg-primary-200 rounded-full blur-2xl opacity-30 animate-pulse" />
                <Image
                  src={imageUrl}
                  alt={name}
                  fill
                  className="relative rounded-full object-cover shadow-2xl"
                  priority
                />
              </div>
            </motion.div>
          )}
        </div>
      </div>
    </section>
  );
}
